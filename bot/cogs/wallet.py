...
ACTIVE_POLLS = set()
ADMIN_ROLE_ID = 1540801378941927525

def has_admin_role():
    """Check whether the interacting user possesses the required staff role."""
    async def predicate(interaction: discord.Interaction) -> bool:
        if not interaction.guild:
            return False
        user_roles = [role.id for role in interaction.user.roles]
        if ADMIN_ROLE_ID not in user_roles:
            raise app_commands.AppCommandError("You do not have permission to use this command.")
        return True
...
class WalletCog(commands.Cog):
    ...
    @app_commands.command(name="addbalance", description="Add or grant balance to a user (Admin only)")
    @app_commands.describe(user="The user to credit", amount="Amount in USD (e.g. 15.50)")
    @has_admin_role()
    async def add_balance(self, interaction: discord.Interaction, user: discord.User, amount: float):
        ...
        await adjust_balance(str(user.id), amount_cents)

        new_total_cents = await get_balance(str(user.id))
        new_total_usd = new_total_cents / 100.0
        ...
        embed.add_field(name="New Total Balance", value=f"**${new_total_usd:.2f}**", inline=False)
        embed.set_footer(text=f"Granted by {interaction.user}")

        await interaction.response.send_message(embed=embed, ephemeral=True)

        try:
            await user.send(f"💳 You have been credited **${amount:.2f}**! Your new balance is **${new_total_usd:.2f}**.")
        except Exception:
            pass

    @add_balance.error
    async def add_balance_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError):
